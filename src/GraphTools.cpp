...
void *worker(void *data){
  ...
  long int size = mydata->n;
  long int j = mydata->j;
  long int k = mydata->k;
  for(int n = 0; n < size; n++){
    //acquire read lock
    pthread_mutex_lock(&distLock);
    if((dist[j][k]+dist[k][n])<dist[j][n]){
      //release write lock
      pthread_mutex_unlock(&distLock);
      //acquire write lock
      pthread_mutex_lock(&distLock);
      dist[j][n]=dist[j][k]+dist[k][n];
      //release write lock
      pthread_mutex_unlock(&distLock);
    }
    else
...
private:
  ...
  public:
    ...
    Floyd(long int size, double* distMap, double* shortestMap, int thread){
      ...
      for(int k = 0; k < size; k++){
        ...
        for(int j = 0; j < size; j++){
          data[j].k = k;
          data[j].j = j;
          pthread_create(threads + j,NULL, worker, (void *)&(data[j]));
        }
