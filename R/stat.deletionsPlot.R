...
stat.deletionsPlot<-function(ant,col.target=NULL,col.random=NULL){
  if(is.null(col.target)){col.target="white"}
  if(is.null(col.random)){col.random="black"}
  par(bg = 'gray63')
  par(mfrow=c(1,3))
  for (a in 1:length(ant)) {
    ...
    plot(ant1[[1]][,1],type="l",col=col.target,ylab=comment(ant1),xlab='Deletions',xaxt='n',
