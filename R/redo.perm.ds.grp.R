...
redo.ds.grp=function(family,new.perm,gbi,oda,odf,target.metrics,formula,Scan,method,ctrlf,fam,...){
  if(new.perm==0){
    ...
    # Glmm test
    if(fam=='gaussian'){model=tryCatch(lme4::lmer(formula=formula, data = new.odf,...), error=identity)}
    #if(family=='nb'){r=tryCatch(lme4::glmer.nb(formula=formula, data = new.odf,family=family,...), error=identity)}
    if(fam!='gaussian'){model=tryCatch(lme4::glmer(formula=formula, data = new.odf,family=family,...), error=identity)}
    
    # Checking error or warnings
    if(isS4(model)){test=c(!is(model,'error'),!is(model,'warning'),model@optinfo$conv$opt==0,length(model@optinfo$conv$lme4$messages)==0,length(model@optinfo$warnings)==0)}
    if(is(model,'error')){test=FALSE}
    if(is(model,'warning')){test=FALSE}
