...
#' @title Outdegree
#' @description Calculate for all the vertices the node metric call met.outdegree.
...
#' @return Integer vector of each vertices met.outdegree.

#' @details  met.indegree of vertice \emph{i} is the number of all edges emmit by node \emph{i}.
...
met.outdegree.single <- function(M, df = NULL, dfid = NULL) {
  ...
  if (is.null(df)) {    
    ...
    if (!is.null(dfid)) {
      ...
      df <- merge.met(vec = BOD, names = colnames(M), df = df, dfid = col.id, met = "outdegree")
      return(df)
    }else{
