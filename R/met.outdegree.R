...
#' @title Outdegree
#' @description Calculates the node metric met.outdegree for all vertices.
...
#' }
#' @details Outdegree of a node \emph{i} is the sum of all the outgoing edges of a node \emph{i}.
...
met.outdegree <- function(M, df = NULL, dfid = NULL) {
  ...
  if (test=="M ok") {
    ...
    if (is.null(df)) {
      ...
      result <- met.outdegree.single(M, df = df, dfid = dfid)
      return(result)
    }
  ...
  if (!is.null(attributes(M)$ANT)) {
    ...
    if (test3) {
      ...
      result <- lapply(M, function(x, df = df, dfid = dfid) {
        r <- met.outdegree.single(x, df = df, dfid = dfid)
  ...
  if (test =="M list ok") {
    ...
    if (is.null(df) & is.null(dfid)) {
      result <- lapply(M, met.outdegree.single)
      return(result)
    }
    ...
    if (!is.null(df) & !is.data.frame(df) & is.list(df)) {
      ...
      if (!is.null(dfid)) {
        # Compute network metric
        result <- mapply(met.outdegree.single, M, df = df, dfid = dfid, SIMPLIFY = FALSE)
        return(result)
      }
      else {
        # Compute network metric
        warning("Argument dfid hasn't been declared. M and df are considered to be ordered exactly in the same way.")
        result <- mapply(met.outdegree.single, M, df = df, SIMPLIFY = FALSE)
        return(result)
      }
    }
  }
