...
{
  if(!is.factor(attr) & !is.character(attr) & !is.numeric(attr)){stop("Argument attr must be a numeric or a vector of factors.")}
  # Parametrization --------------------------------------------------------------------
  ids=colnames(M)
  ...
  if(is.factor(attr) | is.character(attr)){
    ...
    if(!is.character(target)){stop("Argument target must be a character when argument attr is a character or a vector of factors.")}
    if(length(target)!=1){"Argument target must be of length 1. Only one category of individuals can be set as target nodes."}
    if(ndel>length(target.nodes)){stop("Argument ndel is greater than the number of nodes in the target category.")}
    if(nsim>factorial(length(target.nodes))){stop("Argument nsim is greater than the total number of possible combinations of target deletions")}
    ...
    for (j in 1:nsim) {
      ...
      if(return.mat==T){MAT[[1]]=TMAT ; MAT[[2]]=RMAT}
    }
  }
  # Deletions simulation for continuous attributes---------------------------------------------------------------------
  if(is.numeric(attr)){
    if(target!='decreasing' & target!='increasing'){stop("Argument target must be 'decreasing' or 'increasing' when argument attr is numeric.")}
    if(ndel>(length(attr)-2)){stop("Argument ndel must be lower than group size-2")}
    if(!is.numeric(ndel)){stop("Argument ndel must be numeric when argument attr is a numeric vector.")}
    if(length(ndel)!=1){stop("Argument ndel must of length 1.")}
    if(factorial(ndel)<nsim){stop("Argument nsim is higher than argument ndel. You are trying to realize a number of simulations higher than the possible combinations of deletions.")}
    # Selectiing target nodes
