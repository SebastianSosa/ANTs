...
df.to.mat<-function(df,actor,receiver,weighted=NULL,tobs=NULL,sym=F, num.ids=F){
  if(!is.data.frame(df) & is.list(df)){
    if(!is.null(tobs)){
      if(!is.list(tobs)){stop("Argument tobs must a list of vectors when argument df is a list of data frames")}
      m=mapply(function(x,y,actor,receiver,weighted,sym, num.ids){
        ...
        else{ m=m[order(rownames(m)), order(colnames(m))]}
        if(!is.null(y)){
          if(length(y)==ncol(m)){
            m=m/time.heterogeneity(y)
            diag(m)=0
          }
          else{stop("Argument tobs is not of same length as the number of columns in the output matrix.")}
        }
        return(m)
