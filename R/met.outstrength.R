...
met.outstrength <- function(M, df = NULL, dfid = NULL) {
  ...
  if (!is.null(attributes(M)$ANT)) {
    ...
    if (attributes(M)$ANT == "ANT data stream focal sampling multiple matrices") {
      ...
      if (sum(unlist(lapply(seq_along(M), function(i, a) {
        ...
        # data fame manipulation
        if (!is.null(dfid)) {
          dfid <- df.col.findId(df[[1]][[1]], dfid)
          df <- lapply(df, function(x) {
            x <- x[order(x[[dfid]]), ]
          })
        }
  ...
  if (!test & is.list(M)) {
    ...
    if (!is.null(df) & !is.data.frame(df) & is.list(df)) {
      if (!is.null(dfid)) {
        result <- mapply(met.outstrength.single, M, df = df, dfid = dfid, SIMPLIFY = F)
        return(result)
      }
      else {
        if (is.null(dfid)) {
        ...
        result <- mapply(met.outstrength.single, M, df = df, SIMPLIFY = F)
        return(result)
      }
    }
  }
