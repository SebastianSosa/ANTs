...
redo.ds.grp.glm=function(new.perm,gbi,oda,odf,target.metrics,Scan,ctrlf,index,formula, data,family , start, control = list(...), 
  ...
  if(new.perm==0){
    ...
    # LM test
    model=tryCatch(glm(formula=formula, data = new.odf,family=family , start = start, control = control, 
    ...
    if(is(model,'error') | is(model, 'warning')){
      redo.ds.grp.glm(new.perm,gbi,oda,odf,target.metrics,formula,Scan,ctrlf,formula=formula, data = data,family=family , start = start, control = control, 
