                   ...
                   model = TRUE, method = "glm.fit", x = FALSE, y = TRUE, contrasts = NULL, ...) 
{
  ...
  tmp=tryCatch(glm(formula=formula, data=odf,family=family , start = start, control = control, 
          model = model, method = method, x = x, y = y, contrasts = contrasts,...),error=identity)
    ...
    if(test1){
      ...
      if(progress){
        results =lapply(seq_along(ant), function(i,ant,formula,progress=T,ctrl=ctrl,odf,labels,family , start, control, model, method, x, y, contrasts, ...){
          ...
          r=tryCatch(glm(formula=formula, data = ant[[i]],family=family , start = start, control = control, 
                           model = model, method = method, x = x, y = y, contrasts = contrasts,...), error=identity)
          ...
          if(is(r,'error') | is(r,'warning')){
            ...
            while(is(r,'error') | is(r,'warning')){
              ...
              r=tryCatch(glm(formula=formula, data = newdf,family=family , start = start, control = control, 
                               model = model, method = method, x = x, y = y, contrasts = contrasts,...), error=identity)
            }
          }
        ...
        results =lapply(ant, function(d,formula,progress=T,ctrl=ctrl,odf,labels,family , start, control, model, method, x, y, contrasts,...){
          tmp=tryCatch(glm(formula=formula, data = d,family=family , start = start, control = control, 
                           model = model, method = method, x = x, y = y, contrasts = contrasts,...), error=identity)
          ...
          if(is(tmp,'error') | is(tmp,'warning')){
            ...
            while(is(tmp,'error') | is(tmp,'warning')){
              newdf=perm.net.nl(odf,labels,rf=NULL,nperm=1,progress = F)[[2]]
              tmp=tryCatch(glm(formula=formula, data = newdf,family=family , start = start, control = control, 
                               model = model, method = method, x = x, y = y, contrasts = contrasts,...), error=identity)
            }
          }
          r=summary(r)$coefficients[,1]
    ...
    if(test2){
      ...
      if(progress){
        results=lapply(ant, function(d,index, formula, family, start, control, model, method, x, y, contrasts,odf,oda,target.metrics,Scan,ctrlf,..){
          ...
          r=tryCatch(glm(formula=formula, data = d,family=family , start = start, control = control, 
                         model = model, method = method, x = x, y = y, contrasts = contrasts,...),error=identity)
          
          if(is(r,'error') | is(r,'warning')){
            attr(odf,'permutation')=attributes(d)$permutation
            r=redo.ds.grp.glm(new.perm=tmp.env$new.perm, gbi=tmp.env$gbi, oda=oda, odf=odf,target.metrics=target.metrics,Scan=Scan,ctrlf=ctrlf,index=index,formula=formula, data = d,family=family , start = start,control=control, 
