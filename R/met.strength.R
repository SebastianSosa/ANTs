...
#' @title Strength
#' @description Calculates the node metric strentgh for all vertices.
...
#' }
#' @details  Strength of a node \emph{i} is the sum all edge values of the node \emph{i}.
...
met.strength <- function(M, df = NULL, dfid = NULL) {
  ...
  if (test=="M ok") {
    ...
    if (is.null(df)) {
      ...
      result <- met.strength.single(M, df = df, dfid = dfid)
      return(result)
    }
  ...
  if (!is.null(attributes(M)$ANT)) {
    ...
    if (any(test1, test2, test3)) {
      ...
      result <- lapply(M, function(x, df, dfid) {
        r <- met.strength.single(x, df = df, dfid = dfid)
  ...
  if (test =="M list ok") {
    ...
    if (is.null(df) & is.null(dfid)) {
      result <- lapply(M, met.strength.single)
      return(result)
    }
    ...
    if (!is.null(df) & !is.data.frame(df) & is.list(df)) {
      ...
      if (!is.null(dfid)) {
        # Compute network metric
        result <- mapply(met.strength.single, M, df = df, dfid = dfid, SIMPLIFY = FALSE)
        return(result)
      }
      # Check if argument dfid is NULL and print warning
        ...
        # Compute network metric
        warning("Argument dfid hasn't been declared. M and df are considered to be ordered exactly in the same way.")
        result <- mapply(met.strength.single, M, df = df, SIMPLIFY = FALSE)
        return(result)
      }
    }
  }
