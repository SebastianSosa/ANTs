...
stat.glmm2 <- function(ant, formula, family, oda = NULL, progress = T, ncores= NULL, ...) {
  ...
  if (attributes(ant)$ANT == "ANT data stream group sampling multiple matrices") {
    ...
    if(!is.null(ncores)){warning("Parallelization is not supported in datastream protocol.")}
  ...
  if (attributes(ant)$ANT == "ANT node label permutation with random factors") {
    if (fam == "gaussian") {
      ...
      if(is.null(ncores)){
        ...
        cat('Permuted test in parallel starting, no visualization of the progress available.')
        # Parallelisation ---------------------------------------------------------------------------------
        require(parallel)

        cl <- makeCluster(ncores)
        ant=parallel:::splitList(ant, length(cl)) # Split list of data frames according to number of clusters
        permuted <- clusterApply(cl, ant, 
                          fun = function(l, formula, at){
                            tmp.env <- new.env()
                            tmp.env$error <- NULL
                            ctrl <- at$rf
                            lab <- at$labels
                            ...
                            permuted <- lapply(seq_along(l), function(i, l, formula,  ctrl, labels) {
                              r <- tryCatch(suppressWarnings(lme4::lmer(formula = formula, data = l[[i]])), error = identity)
                              if (isS4(r)) {
                                r2=with(r@optinfo$derivs,solve(Hessian,gradient))
                                if(max(abs(r2))<0.001){test=TRUE}
                                else{
                              ...
                              if (all(test) != TRUE) {
                                #tmp.env$error <- i
                                while (all(test) != TRUE) {
                                  newdf <- ant:::perm.redo(df = odf, labels = labels, ctrl = ctrl)
                                  r <- tryCatch(suppressWarnings(lme4::lmer(formula = formula, data = newdf)), error = identity)
                              ...
                              #return(list('permuted'=r, 'error'=error))
                              return(r)
                            }, l=l, formula=formula, ctrl = ctrl, labels = labels)
                          }, formula=formula, at=at)
        
        permuted=do.call(c, permuted, quote = TRUE)
        
        #z=do.call(c, z, quote = TRUE)
        #z=do.call(c, z, quote = TRUE)
        
        # Extracting errors
        #error=z[which(names(z)=='error')]
        #error=do.call(c, error)
        
        # Extracting permuted estimates
        #permuted=z[which(names(z)=='permuted')]
      }
    }
      ...
      if(is.null(ncores)){
        ...
        permuted <- clusterApply(cl, ant, 
                                 fun = function(l, formula, at){
                                   ...
                                   permuted <- lapply(seq_along(l), function(i, l, formula,  ctrl, labels) {
                                     r <- tryCatch(suppressWarnings(lme4::glmer(formula = formula, data = l[[i]], family = family)), error = identity)
                                     if (isS4(r)) {
                                       r2=with(r@optinfo$derivs,solve(Hessian,gradient))
                                       if(max(abs(r2))<0.001){test=TRUE}
                                       else{
    ...
    else{result <- list("Original.model" = obs, "permutations" = permuted)}
    attr(result, "class") <- "ant glmm"
    attr(result, "family") <- paste(family)
    attr(result, "formula") <- format(formula)
    cat("\n")
