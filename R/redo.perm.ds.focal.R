...
redo.ds.focal.glmm<-function(family,formula,new.perm,gbi,gbi2,oda,odf,target.metrics,focal,ctrl,alters,index,fam,...){
  if(new.perm==0){
    ...
    # Glmm test
    if(fam=='gaussian'){r=tryCatch(lme4::lmer(formula=formula, data = odf.tmp,...), error=identity)}
    #if(family=='nb'){r=tryCatch(lme4::glmer.nb(formula=formula, data = new.odf,family=family,...), error=identity)}
    if(fam!='gaussian'){r=tryCatch(lme4::glmer(formula=formula, data = odf.tmp,family=family,...), error=identity)}
    
    # Checking error or warnings
    if(isS4(r)){test=c(!is(r,'error'),!is(r,'warning'),length(r@optinfo$conv$lme4$messages)==0)}
