...
if (!is.matrix(Matrix)){stop("Argument Matrix is not a matrix")}
if (ncol(Matrix)!=length(values)){stop("number of elements in 'values' is not equal to number of nodes")}
if (weighted){MATjk<-Matrix} else{
  MATjk<-Matrix
  MATjk[MATjk > 0]<- 1
  MATjk[MATjk < 1]<- 0}
  ...
  if (se){
    ...
    if(weighted){
      if(is.null(df)){return(c("assor.conti.w" = r, "assor.se" = se))}
      else{
        df$assor.conti.w=r
        df$assor.se=se
      }
      ...
      if(is.null(df)){return(c("assor.conti.b" = r, "assor.se" = se))}
      else{
        df$assor.conti.b=r
        df$assor.se=se
      }
